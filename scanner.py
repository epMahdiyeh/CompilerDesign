...
def lookahead_state(state: int, char: str):
    ...
    # num
        ...
        if (re.match( '[0-9]',char)):
            position = 3
        elif ((char in whitespace + symbol + ["=", "*", "/"]) or (re.match( '[a-zA-z]',char))):
            position = 16
            end = True
        else:
    ...
    # symbol
        ...
        if (char in whitespace + ["=", "*", "/"] or re.match( '[a-zA-Z0-9]',char)):
            position = 18
        else:
            error = True
            position = "invalid input"
...
def get_next_token(INPUT, KEYWORDS):
    states = ["", "", "", "", "", "", "symbol", "", "", "", "", "", "", "whitespace", "ID", "Num", "symbol", "symbol",
              "", "comment", "comment", "", "", "symbol" "ID", "", "NUM", "SYMBOL", "", "SYMBOL", "", "SYMBOL", "", "",
              "COMMENT", "", "", "WHITESPACE"]
    change_line = 0
    string = ""
